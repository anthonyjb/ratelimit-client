...
# @@ START HERE
#
# - Define the concept of top level types terrain, feature, landmark, creature,
#   etc along with the basic strucuture tree, oak, great oak (type, species
#   and sub-species).
# - Scene class for loading and rendering scenes
# - Update the in-game to support switing to an in-scene state
#

# @@ Add a new state when joining a game which allows you to specify or change
#    your name.
#
# - get endpoint in place to check if I am already a player in the game
# - if not put in place a scene that asks the player for their name
#       - consider a stepped approach with sub-scenes so that we can build the
#         basics for onboarding
# - on joining I must provide a name
#
...
Stats we need to show on the UI
...
- You
    - Your name (text_number)
    - Level (number)
    - Stamina (number_of)
    - Health (numbers_of)
    - Exp (numbers_progress)
